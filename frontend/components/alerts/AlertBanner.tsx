...
import { useAlerts, Alert } from './GlobalAlertProvider';
...
function AlertComponent({ alert, onClose }: AlertProps) {
  const { id, type, message, title } = alert;

  // Determine the color scheme based on the alert type
  ...
  return (
    <div className={`border-l-4 p-4 rounded-md mb-4 ${getColorScheme()}`}>
      <div className="flex items-start">
        ...
        <div className="ml-3 w-full">
          <div className="flex justify-between items-center">
            ...
            <button
              type="button"
              className="ml-auto -mx-1.5 -my-1.5 rounded-md focus:outline-none"
              onClick={() => onClose(id)}
            >
...
export default function AlertBanner() {
  ...
  return (
    <div className="fixed top-5 right-5 z-50 w-96 max-w-full space-y-2">
      {alerts.map((alert) => (
